...
library(DescTools)
library(PMCMRplus)
setwd("/Users/farshadkazemi/Downloads")
...
df <- data.frame(threshold=factor(levels = unique(data$threshold)),
             ...
             pvalue=double(),
             type=factor(levels=c("dynamic vs norm","dynamic vs static","norm vs static")),
...
for (prj in prjs) {
  for (thr in thrs) {
    ...
    #f=res.friedman_test$statistic
    #k=dim(subData)[2]
    #nf=length(subData[,1])
    #res.conover<-ConoverTest(subData$res ~ subData$method | subData$PeriodId,method = "holm")
    ...
    df[nrow(df) + 1,] = c(thr,prj,pvalues[1,1],"dynamic vs norm")
    df[nrow(df) + 1,] = c(thr,prj,pvalues[2,1],"dynamic vs static")
    df[nrow(df) + 1,] = c(thr,prj,pvalues[2,2],"norm vs static")
